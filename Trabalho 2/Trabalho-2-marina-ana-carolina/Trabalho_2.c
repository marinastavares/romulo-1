...
void escreveArquivo(){
	...
	while(1){
		...
		FILE* arquivo = fopen("Trab2_final.txt","a+");

		if (buffer_descarregado == 0){
			int i;
			for (i =0 ; i < TAM_MEU_BUFFER ; i++){
				char conteudo[20]="";
				sprintf(conteudo,"Valor do buffer 1 %ld: %f  Data: %d  Hora:%d:%d \n", ++leitura, buffer1[i], data_hora_atual->tm_mday, data_hora_atual->tm_hour,data_hora_atual->tm_min, data_hora_atual->tm_sec);
			...
			for (i =0 ; i < TAM_MEU_BUFFER ; i++){
				char conteudo[20]="";
				sprintf(conteudo,"Valor do buffer 2 %ld: %f  Data: %d  Hora:%d:%d \n", ++leitura, buffer2[i], data_hora_atual->tm_mday, data_hora_atual->tm_hour,data_hora_atual->tm_min, data_hora_atual->tm_sec);				fputs(conteudo,arquivo);
			}
		}
		fclose(arquivo);
		buffer_cheio = 0;
